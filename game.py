...
err=0
def pointer():
    global timer_started, start_time, err, prev_button, score,selected_floor, current_floor, lift_y, go_clicked, feedback_text, val
    pos = findPose()
    ...
    if pointer_ellipse.colliderect(go_rect):
        if not go_clicked:
            if selected_floor == -1:
                ...
                current_floor = random.randint(1, num_floors)  # Update the current_floor only when the selected floor is correct
                score += 1
                err=0
            else:
                feedback_text = "Try Again"  # Red text color
                # Do not update val here
                err+=1
                if err == 3:
                    # Save username and score to score.csv
        ...
        if feedback_text == "Try Again":
            result_text = big_font.render(feedback_text, True, (255, 0, 0))
        else: 
            result_text = big_font.render(feedback_text, True, (0, 255, 0))
        result_text_rect = result_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT-100))
...
def draw_start_menu():
    ...
    while True:
        ...
        screen.fill(WHITE)
        pygame.draw.rect(screen, BLACK, input_box, 2)  # White border
        username_text = font.render(username, True, BLACK)  # White text color
...
with mp_hands.Hands(min_detection_confidence=0.8, min_tracking_confidence=0.5) as hands:
    while running:
        ...
        score_background = pygame.Rect(SCREEN_WIDTH - 500, SPACING, 200, 50)  # Adjust the size (200, 50) as needed
        pygame.draw.rect(screen, (255, 0, 0), score_background)  # Draw a red rectangle

        score_text = font.render(f'Score: {score}', True, (255, 255, 255))  # White text color
        score_text_rect = score_text.get_rect(topright=(SCREEN_WIDTH - 350, SPACING))
